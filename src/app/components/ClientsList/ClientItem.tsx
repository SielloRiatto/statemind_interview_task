...
export default function ClientItem({
  ...
  const itemClassName = useMemo(() => (`
    ...
    ${!isLast && 'pb-8 after:absolute after:top-[calc(1.875rem+1px)] after:bottom-0 after:left-[0.6875rem] after:w-px after:bg-slate-200 dark:after:bg-slate-200/5'}
  `), [isLast])
  ...
  return (
    <li className={itemClassName} style={{ counterIncrement: "step 1"}}>
      ...
      <div className="peer group flex-auto ml-4">
        <div>{name} ({reportsCount})</div>
        <Link href={linkHref} className={linkClassName}>See projects</Link>
      </div>
    </li>
  )
}